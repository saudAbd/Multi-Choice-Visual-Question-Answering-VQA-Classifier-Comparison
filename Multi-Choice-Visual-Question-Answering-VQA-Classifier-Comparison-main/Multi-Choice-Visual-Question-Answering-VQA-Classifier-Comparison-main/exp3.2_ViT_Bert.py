    ...
    def load_data(self):
        with open(self.data_file) as f:
            ...
            for line in lines:
                ...
                # Split the text only on the first occurrence of '?'
                split_text = text.split("?", 1)
                if len(split_text) == 2:  # If there is exactly one '?'
                    question, answer = split_text
                    question = question.strip() + "?"  # Ensure that the question ends with '?'
                    answer = answer.strip()
                else:
                    # Handle the case where no '?' is found (or more than one '?')
                    question = split_text[0].strip()  # Take the whole text as question if no answer
                    answer = ""  # Empty answer if the format is invalid
...
class ITM_Model(nn.Module):
    def __init__(self, text_dim=768, img_dim=768, hidden_dim=128, num_classes=2):
        super().__init__()
        
        # Use ViT as the image encoder
    ...
    def forward(self, img, q_emb, a_emb):
        # Get the [CLS] token representation from ViT output
        img_feat = self.vision_model(img).last_hidden_state[:, 0, :]  # Shape: [batch_size, hidden_dim]
...
def train_model(model, train_loader, criterion, optimizer, num_epochs=30):
    ...
    for epoch in range(num_epochs):
        total_loss = 0.0
        for img, q_emb, a_emb, labels in tqdm(train_loader, desc=f"Epoch {epoch + 1}/{num_epochs}", ncols=100, dynamic_ncols=True):
